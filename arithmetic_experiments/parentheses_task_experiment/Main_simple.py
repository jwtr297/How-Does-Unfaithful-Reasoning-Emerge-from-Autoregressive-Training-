...
from arithmetic_experiments.parentheses_task_experiment.Multiplication_addition_training_simple import make_scheduler, train_with_dataloader, save_results  ## CHANGED
...
def main(p1, p2, batch_limit: int = 62500, num_epochs: int = 16, modulus: int = 11, **kwargs):
    ...
    for i_p1, p1_val in enumerate(p1_grid):
        ...
        for j_p2, p2_val in enumerate(p2_grid):
            ...
            # --------- load dataset ----------
            data_dir = f"./tasks/parentheses_task/3steps_multiplication_addition_dataset/mod{modulus}_{p1_val:.3f}_{p2_val:.3f}.pt"
            ...
            # --------- train ----------
            model, training_info, hiddenstates_tensor = train_with_dataloader(
                model,
                config,
                optimizer,
                scheduler,
            ...
            # --------- save hiddenstates info into the p1 folder (filename carries p2) ----------
            hiddenstates_name = f"{config.num_layers+1}layers{config.num_heads}heads_p2{fmt(p2_val)}_hiddenstates.pt"
            torch.save(hiddenstates_tensor, os.path.join(p1_dir, hiddenstates_name))
