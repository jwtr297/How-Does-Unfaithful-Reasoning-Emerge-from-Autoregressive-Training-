...
from arithmetic_experiments.parentheses_task_experiment.utils import matching_scores, TrainingInfo, autoregressive_extend, induction_score, simplification_head_score, eval_opposite_structure
from tasks.parentheses_task.modular_data_generation import generate_noisy_twosteps_dataset_modp, generate_noisy_twosteps_dataset_modp_1, ModularVocabulary,compute_e1_e2_distributions, compute_short_E3_distributions, generate_noisy_twosteps_varied_dataset_modp
from pathlib import Path
from arithmetic_experiments.parentheses_task_experiment.unfaithfulness_metrics import measure_e2_all_digits_intervention_ce_modp, measure_e2_all_digits_intervention_ce_modp_short, measure_e1_ab_digits_intervention_ce_modp_short, compute_e3_entropy_sharpness_modp_short,measure_e3_agreement_with_e1_e2_modp, autoreg_generate_from_firstded_to_stop_1, autoreg_generate_from_secondded_to_stop_1, proportion_E1_eq_r_but_E2_neq_r_1,third_layer_attn_ffn_no_residual, shuffle_e2_first_digit_modp_short, get_last_layer_QK, select_E1_a_in_firstN, proportion_E2_eq_r_1, proportion_E1_eq_r_1, select_E1_firstdigit_and_E1_not_eq_E2, select_E1_not_eq_E2, measure_e2_addup_intervention_ce_modp_short, proportion_E1_eq_r_but_E2_neq_r_1, proportion_E1_neq_r_but_E2_eq_r_1, proportion_E1_eq_r_and_E2_eq_r_1, proportion_E1_neq_r_and_E2_neq_r_1, compute_structure_accuracy_from_first_deduct_seq, proportion_E2_E3_joint_cases


def build_token_display_map(modulus: int, vocab: ModularVocabulary | dict | None = None) -> dict:  
...
def get_weighted_loss(
    ...
    targets = src[:, 1:].contiguous().view(-1)           
    ...
    # position mask
    if mask is not None:
        pos_mask = mask[:, :-1].to(per_tok_loss.device).float()  
    else:
        pos_mask = torch.ones((B, T-1), device=per_tok_loss.device, dtype=torch.float)
    sw = sample_weights.to(per_tok_loss.device).float().unsqueeze(1).expand(B, T-1)  

    if sampling_mode == "uniform":
        eff_w = (sw * pos_mask).reshape(-1)                
        ...
        per_sample_ce = (tok_loss * pos_mask).sum(dim=1) / denom  
...
def train_with_dataloader(
    ...
    train_loader, N=1000, p1=0.05, p2=0.05, training_info=None, modulus: int = 11, vocab: ModularVocabulary | dict | None = None, shortcut_evaluation=False  ## CHANGED
):

    test_set, test_vocab, test_set_info = generate_noisy_twosteps_dataset_modp_1(
        N, modulus, p1, p2, device=config.device
    )  


    #test_pos, test_pos_vocab, test_pos_info = generate_noisy_twosteps_dataset_modp_1(  
    #    N, modulus, 0, 1, device=config.device
    #)
    
    test_neg, test_neg_vocab, test_pos_info = generate_noisy_twosteps_dataset_modp_1(  
    ...
    if shortcut_evaluation:
        test_set_fullnoise, _, _ = generate_noisy_twosteps_varied_dataset_modp(  
        N*100, modulus, p1, p2, device=config.device
    ) 
    
        test_set_shortcut = select_E1_firstdigit_and_E1_not_eq_E2(test_set_fullnoise,[0,2,int(modulus/2)],modulus, N)
    ...
    for epoch, (src_cpu, mask_cpu) in enumerate(
        ...
        if evaluation_ready:  ## CHANGED
            ...
            with torch.no_grad():
                ...
                avg_KL, _, avg_inv,_ = measure_e2_all_digits_intervention_ce_modp_short(model, test_set, modulus)
                
                
                avg_KL_1, _, avg_inv_1,_ = measure_e1_ab_digits_intervention_ce_modp_short(model, test_set, modulus)
                ...
                ratio_E2_eq_E3_eq, ratio_E2_eq_E3_neq, ratio_E2_neq_E3_eq, ratio_E2_neq_E3_neq = proportion_E2_E3_joint_cases(pred_autoreg,test_set,modulus)



                if shortcut_evaluation == True:
                    pred_autoreg_shortcut = autoreg_generate_from_firstded_to_stop_1(model, test_set_shortcut, p=modulus) 
                    ratio_E2_eq_E3_eq_shortcut, ratio_E2_eq_E3_neq_shortcut, ratio_E2_neq_E3_eq_shortcut, ratio_E2_neq_E3_neq_shortcut = proportion_E2_E3_joint_cases(pred_autoreg_shortcut, test_set_shortcut, modulus)
                    avg_KL_shortcut, _, avg_inv_shortcut,_ = measure_e2_all_digits_intervention_ce_modp_short(model, test_short_cut, modulus)

                

                
                E1_pred, E2_pred, Mixed_pred = compute_short_E3_distributions(test_set, modulus, p1 ,p2)
                
                target_guess_pred_og = e1e2_indicator_target(test_set, modulus)
                
                logits,_ = model(pred_autoreg)
                model_prob = F.softmax(logits[:, -3, :][:, :modulus], dim=-1)
                
                KL_E1 = batch_kl_mean(E1_pred, model_prob)
                KL_E2 = batch_kl_mean(E2_pred, model_prob)
                KL_Mixed = batch_kl_mean(Mixed_pred, model_prob)
                KL_targeted = batch_kl_mean(target_guess_pred_og, model_prob)

                
                
                #2. we use the original and use the generated E2 to get the metric
                E1_pred_og, E2_pred_og, Mixed_pred_og = compute_short_E3_distributions(test_set, modulus, p1 ,p2)
                
                

                logits_og,_ = model(test_set)
                model_prob_og = F.softmax(logits_og[:, -3, :][:, :modulus], dim=-1)
                
                
                KL_E1_og = batch_kl_mean(E1_pred_og, model_prob_og)
                KL_E2_og = batch_kl_mean(E2_pred_og, model_prob_og)
                KL_Mixed_og = batch_kl_mean(Mixed_pred_og, model_prob_og)
                KL_targeted_og = batch_kl_mean(target_guess_pred_og, model_prob_og)


                #format score accuracy
                format_score = compute_structure_accuracy_from_first_deduct_seq(pred_autoreg, test_set, modulus)
                

                #self relection index
                hidden_state_pos = third_layer_attn_ffn_no_residual(model, test_pos.long()[0:1000], use_training_mode=False)

                hidden_state_neg = third_layer_attn_ffn_no_residual(model, test_neg.long()[0:1000], use_training_mode=False)

                #1. indicator 
                indicator_metric = -(hidden_state_pos[:,-3,:].abs().amax(dim=1)-hidden_state_neg[:,-3,:].abs().amax(dim=1)).mean()
                

                delta_E3 = (hidden_state_pos[:,-3,:]-hidden_state_neg[:,-3,:]).mean(dim=0)
                hiddenstates_rows.append(delta_E3)


                #2. QK nuclear norm
                QK_pos = get_last_layer_QK(model, test_pos)
                QK_neg = get_last_layer_QK(model, test_neg)
            
                svdvals = torch.linalg.svdvals((QK_pos-QK_neg)[0])
                lambda_max = svdvals.max()
                nuclear_norm = svdvals.sum()
                
                
                svdvals_head2= torch.linalg.svdvals((QK_pos-QK_neg)[1])
                lambda_max_head2 = svdvals_head2.max()
                nuclear_norm_head2 = svdvals_head2.sum()
            ...
            if run is not None:  ## CHANGED
                ...
                metrics = {"loss": loss_test, "error_sentence_level": test_err_1, "error_element_level": test_err_2, "Model_Prediction_Accuracy": result_acc, "Entropy_of_Model’s_Predicted_Distribution": sharpness,
"Entropy_of_Model’s_Predicted_Distribution_E2_pred": sharpness_pred, "Format_score": format_score, "INR_under_E2_intervention": avg_inv, "E2_IDS": avg_KL, "INR_under_E1_intervention": avg_inv_1, "E1_Intervention_KL_Divergence": avg_KL_1, "HSC": indicator_metric, "AC_1": lambda_max,"AC_2": lambda_max_head2,
"INR_E2_intervention_Partial": avg_inv_partial, "E2_IDS_Partial": avg_KL_partial, "ratio_E2e_E3n": ratio_E2_eq_E3_neq, "ratio_E2n_E3e": ratio_E2_neq_E3_eq, "ratio_E2e_E3e": ratio_E2_eq_E3_eq, "ratio_E2n_E3n": ratio_E2_neq_E3_neq, "KL_E1_pred": KL_E1, "KL_E2_pred": KL_E2, "KL_mixed_pred": KL_Mixed, "KL_targeted_pred": KL_targeted, "KL_E1": KL_E1_og, "KL_E2": KL_E2_og, "KL_mixed": KL_Mixed_og, "KL_targeted": KL_targeted_og}
                
                if shortcut_evaluation:
                    metrics.update({"INR_under_E2_intervention_shortcut": avg_inv_shortcut, "E2_IDS_shortcut": avg_KL_shortcut, "ratio_E2e_E3n_shortcut": ratio_E2_eq_E3_neq_shortcut, "ratio_E2n_E3e_shortcut": ratio_E2_neq_E3_eq_shortcut, "ratio_E2e_E3e_shortcut": ratio_E2_eq_E3_eq_shortcut, "ratio_E2n_E3n_shortcut": ratio_E2_neq_E3_neq_shortcut})
        ...
        if len(hiddenstates_rows) > 0:
            hiddenstates_tensor = torch.stack(hiddenstates_rows, dim=0)  # shape: [M, L]
        else:
            hiddenstates_tensor = torch.empty((0, L), dtype=torch.long, device=your_device)
    ...
    return model, training_info, hiddenstates_tensor
